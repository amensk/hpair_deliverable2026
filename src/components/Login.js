import React, { useState } from 'react';
import { Formik, Form } from 'formik';
import * as Yup from 'yup';
import { FiEye, FiEyeOff, FiArrowRight, FiArrowLeft, FiMail } from 'react-icons/fi';
import { signInUser, registerUser, resetPassword } from '../services/authService';
import { TextField } from './ui/Field';
import Button from './ui/Button';
import Alert from './ui/Alert';

const emailRule = Yup.string().trim().email('Enter a valid email address').required('Email is required');

const schemaFor = (mode) => {
  if (mode === 'reset') return Yup.object({ email: emailRule });
  const isLogin = mode === 'login';
  return Yup.object({
    email: emailRule,
    password: Yup.string()
      .required('Password is required')
      .min(6, 'Password must be at least 6 characters')
      .when([], { is: () => !isLogin, then: (s) => s.matches(/\d/, 'Include at least one number') }),
    confirm: isLogin
      ? Yup.string().strip()
      : Yup.string().oneOf([Yup.ref('password')], 'Passwords do not match').required('Confirm your password'),
  });
};

const COPY = {
  login: { title: 'Sign in', sub: 'Use the email and password you registered with.', cta: 'Sign in' },
  register: { title: 'Create your account', sub: 'One account per delegate. You can sign back in any time.', cta: 'Create account' },
  reset: { title: 'Reset your password', sub: "Enter your email and we'll send a link to choose a new password.", cta: 'Send reset link' },
};

const Login = () => {
  const [mode, setMode] = useState('login'); // 'login' | 'register' | 'reset'
  const [showPw, setShowPw] = useState(false);
  const [message, setMessage] = useState(null);
  const [lastEmail, setLastEmail] = useState('');

  const switchMode = (next) => {
    setMode(next);
    setMessage(null);
  };

  const handleSubmit = async (values, { setSubmitting }) => {
    setMessage(null);
    const email = values.email.trim();
    setLastEmail(email);
    let result;
    if (mode === 'login') result = await signInUser(email, values.password);
    else if (mode === 'register') result = await registerUser(email, values.password);
    else result = await resetPassword(email);

    if (mode === 'reset') setMessage({ type: result.success ? 'success' : 'error', text: result.message });
    else if (!result.success) setMessage({ type: 'error', text: result.message });
    // On successful sign-in/register the AuthProvider swaps this screen for the form.
    setSubmitting(false);
  };

  const copy = COPY[mode];

  return (
    <div className="auth">
      <div className="stat-band stat-band--wide">
        <div><strong>42+</strong><span>Countries Represented</span></div>
        <div><strong>40,000+</strong><span>Past Delegates</span></div>
        <div><strong>500+</strong><span>Past Speakers</span></div>
      </div>

      <section className="auth__form container">
        <div className="auth__card card card--pad">
          {mode === 'reset' && (
            <button type="button" className="btn btn--ghost btn--sm" style={{ width: 'auto', marginBottom: 12, marginLeft: -12 }} onClick={() => switchMode('login')}>
              <FiArrowLeft size={16} aria-hidden="true" /> <span>Back to sign in</span>
            </button>
          )}
          <h2>{copy.title}</h2>
          <p style={{ marginTop: 6, marginBottom: 24 }}>{copy.sub}</p>

          {message && <Alert type={message.type}>{message.text}</Alert>}

          <Formik
            key={mode}
            initialValues={{ email: lastEmail, password: '', confirm: '' }}
            validationSchema={schemaFor(mode)}
            validateOnChange
            validateOnBlur
            onSubmit={handleSubmit}
          >
            {({ isSubmitting, isValid, dirty }) => (
              <Form noValidate>
                <TextField name="email" label="Email address" type="email" autoComplete="email" placeholder="you@university.edu" autoFocus />
                {mode !== 'reset' && (
                  <div className="field">
                    <TextField
                      name="password"
                      label="Password"
                      type={showPw ? 'text' : 'password'}
                      autoComplete={mode === 'login' ? 'current-password' : 'new-password'}
                      placeholder={mode === 'login' ? 'Your password' : 'At least 6 characters, with a number'}
                    />
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: -12, flexWrap: 'wrap', gap: 8 }}>
                      <button type="button" className="btn btn--ghost btn--sm" style={{ width: 'auto' }} onClick={() => setShowPw((s) => !s)} aria-pressed={showPw}>
                        {showPw ? <FiEyeOff size={16} aria-hidden="true" /> : <FiEye size={16} aria-hidden="true" />}
                        <span>{showPw ? 'Hide password' : 'Show password'}</span>
                      </button>
                      {mode === 'login' && (
                        <button type="button" className="auth__link" onClick={() => switchMode('reset')}>
                          Forgot password?
                        </button>
                      )}
                    </div>
                  </div>
                )}
                {mode === 'register' && (
                  <TextField name="confirm" label="Confirm password" type={showPw ? 'text' : 'password'} autoComplete="new-password" />
                )}
                <Button
                  type="submit"
                  className="btn--block"
                  loading={isSubmitting}
                  disabled={(!dirty && !lastEmail) || !isValid}
                  iconRight={mode === 'reset' ? undefined : FiArrowRight}
                  icon={mode === 'reset' ? FiMail : undefined}
                >
                  {copy.cta}
                </Button>
              </Form>
            )}
          </Formik>

          {mode !== 'reset' && (
            <p className="auth__toggle">
              {mode === 'login' ? "Don't have an account? " : 'Already registered? '}
              <button type="button" onClick={() => switchMode(mode === 'login' ? 'register' : 'login')}>
                {mode === 'login' ? 'Create one' : 'Sign in instead'}
              </button>
            </p>
          )}
        </div>
      </section>
    </div>
  );
};

export default Login;
